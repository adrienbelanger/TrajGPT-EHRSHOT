...
class TrajGPT(nn.Module):
    ...
    def __init__(self, configs, head_type='pretrain', forecast_method='time_specific', n_output=2):
        ...
        self.token_embedding = nn.Embedding(
            num_embeddings=configs.vocab_size,
            embedding_dim=configs.d_model,
            padding_idx=0  # [PAD] is index 0 in our vocab
        )
        ...
        if self.head_type == "pretrain":
            self.n_output = configs.vocab_size  # Update n_output to the number of unique PheCodes
    ...
    def forward(self,
        ...
        hidden_states = self.token_embedding(X) # batch_size x seq_len x d_model
    ...
    def compute_pretrain_loss(self, logits, targets):
        ...
        """
        self.ce_loss = nn.CrossEntropyLoss(ignore_index=0)  # 0 is [PAD]
        logits = logits.view(-1, logits.size(-1))
        targets = targets.view(-1).long().to(logits.device)
